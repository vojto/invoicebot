...
class InvoiceExtractionAgent < ApplicationAgent
  ...
  def call
    ...
    result_for([ extract_from_pdf(source_path) ])
  ...
  def extract_from_pdf(pdf_path)
    result = ask(BASE_USER_PROMPT, schema: ResponseSchema, attachment: pdf_path)

    Rails.logger.info "[InvoiceExtractionAgent] AI response received for #{FULL_PDF_SCOPE} in #{result.elapsed_ms}ms"
